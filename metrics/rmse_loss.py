...
class RmseLoss(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        self.register_buffer('min_rate', torch.tensor(0.001, dtype=torch.float32)) 
        #self.register_buffer('av_batch_size', None) 
    # END RmseLoss.__init__
    ...
    def forward(self, pred, target, data_filters=None ):        
        ...
        if data_filters is None:
            ...
            loss_full = torch.mul(self.lossNR(pred, target), data_filters)
        ...
        loss_weighting = torch.reciprocal( torch.maximum(pred, self.min_rate) )
        #loss = torch.mean(loss_full * loss_weighting)
