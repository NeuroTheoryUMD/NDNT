...
from .convlayers import STconvLayer, ConvLayer
...
class MaskLayer(NDNLayer):
    ...
    def set_mask( self, mask=None, replacement=None ):
        ...
        if mask is None:
            ...
            assert np.prod(mask.shape) == np.prod(self.filter_dims)*self.num_filters, "Mask dimensions incorrect"
            self.mask = torch.tensor( mask.reshape([-1, self.num_filters]), dtype=torch.float32)

        if replacement is not None:
            assert np.prod(replacement.shape) == np.prod(self.filter_dims)*self.num_filters, "Replacement dimensions incorrect"
...
class MaskConvLayer(ConvLayer):
    ...
    def layer_dict(cls, **kwargs):
        ...
        Ldict = super().layer_dict(**kwargs)
        # Added arguments
        Ldict['layer_type'] = 'maskconv'
        #Ldict['mask'] = mask    
        return Ldict
    # END MaskConvLayer.layer_dict()
